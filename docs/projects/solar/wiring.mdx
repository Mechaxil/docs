...
Battery- -> Waveshare BAT-
Waveshare 5V/3.3V out -> Buck (if needed) -> Pico/<GlossaryTerm term="ESP32" /> power
Battery + -> <GlossaryTerm term="MOSFET" /> V+ ; <GlossaryTerm term="MOSFET" /> drain -> Solenoid + ; Solenoid - -> <GlossaryTerm term="MOSFET" /> source -> Battery - (common ground)
```
